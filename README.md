...
| **Linux** | Working, and the most complete backend. T0–T3 **measured on kernel 7.2.2 (Landlock ABI v10)**: 29/29 tests, 30 adversarial escape attempts, **0 escapes and 0 documented limits**. Observation is unprivileged (seccomp user-notification) and PID/IPC isolation is real. See [`docs/linux-bringup.md`](docs/linux-bringup.md). |
...
cmake --build build
(cd build && ctest --output-on-failure)     # 29/29
